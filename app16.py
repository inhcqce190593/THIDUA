...
def tong_ket():
    ...
    #     ))
    def tong_ket_lop(lop_truc, tuan):
    # """Tính & lưu tổng kết cho 1 lớp; tự thêm cột 'lop' nếu bảng bang_tong_ket có cột này."""
    # 1) Lấy khoi + lop từ phân công
        cursor.execute(
            "SELECT khoi, lop FROM phan_cong_truc WHERE lop_truc = %s AND tuan = %s",
            (lop_truc, tuan)
        )
        pc = cursor.fetchone()
        cursor.fetchall()
        khoi = (pc or {}).get('khoi') or 'Unknown'
        lop  = (pc or {}).get('lop')    # có thể None nếu chưa lưu trong phan_cong_truc

        # 2) Tính điểm học tập & nội quy
        cursor.execute(
            "SELECT COALESCE(SUM(tong_diem),0) AS total_study_points FROM study_data WHERE tuan = %s AND lop_truc = %s",
            (tuan, lop_truc)
        )
        study_points = (cursor.fetchone() or {}).get('total_study_points', 0)
        cursor.fetchall()

        cursor.execute(
            "SELECT COALESCE(SUM(tong_diem_vi_pham),0) AS total_rules_points FROM rules_data WHERE tuan = %s AND lop_truc = %s",
            (tuan, lop_truc)
        )
        rules_points = (cursor.fetchone() or {}).get('total_rules_points', 0)
        cursor.fetchall()

        tong_diem_chung = (study_points or 0) + (rules_points or 0)

        # 3) Bảng bang_tong_ket có cột 'lop' không?
        # cursor.execute("""
        #     SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
        #     WHERE TABLE_SCHEMA = DATABASE()
        #     AND TABLE_NAME = 'bang_tong_ket'
        #     AND COLUMN_NAME = 'lop'
        # """)
        ...
        has_lop_col = (row.get('cnt', 0) == 1)
        cursor.fetchall()

                
        # has_lop_col = (cursor.fetchone() or [0])[0] == 1
        # cursor.fetchall()

        # 4) Ghi dữ liệu - có/không có cột 'lop'
        if has_lop_col:
            # Bắt buộc phải có 'lop' -> nếu không có trong phân công thì fallback = lop_truc
            val_lop = lop if lop is not None else str(lop_truc)
            cursor.execute("""
                INSERT INTO bang_tong_ket
                    (tuan, khoi, lop, lop_truc, tong_diem_hoc_tap, tong_diem_noi_quy, tong_diem_chung, trangthai)
                VALUES (%s,   %s,   %s,  %s,       %s,                  %s,                 %s,              %s)
                ON DUPLICATE KEY UPDATE
                    khoi = VALUES(khoi),
                    lop = VALUES(lop),
            ...
            """, (tuan, khoi, val_lop, lop_truc, study_points, rules_points, tong_diem_chung, 'Đã tổng kết'))
        else:
            cursor.execute("""
                INSERT INTO bang_tong_ket
                    (tuan, khoi, lop_truc, tong_diem_hoc_tap, tong_diem_noi_quy, tong_diem_chung, trangthai)
                VALUES (%s,   %s,   %s,       %s,                  %s,                 %s,              %s)
                ON DUPLICATE KEY UPDATE
                    khoi = VALUES(khoi),
                    tong_diem_hoc_tap = VALUES(tong_diem_hoc_tap),
                    ...
                    tong_diem_chung = VALUES(tong_diem_chung),
                    trangthai = VALUES(trangthai)
            """, (tuan, khoi, lop_truc, study_points, rules_points, tong_diem_chung, 'Đã tổng kết'))
...
def add_account():
    ...
    input_lop = request.form.get('lop', '').strip()
    input_capquanli = request.form.get('Capquanli', '').strip()

    # Sinh mật khẩu và map role theo Capquanli như code cũ
    input_password = generate_specific_password()
    input_role = input_capquanli or 'user'

    # Chuẩn hóa để tránh NOT NULL lỗi (nếu form trống)
    vals = {
        "Name":       input_name or "",
        "Username":   input_username or "",
        "Password":   input_password or "",
        "Role":       input_role or "",
        "Lop":        input_lop or "",
        "Tuan":       input_tuan,
        "Capquanli":  input_capquanli or "",
        "Lop_truc":   "",                 # mặc định rỗng; nếu form có thì thay bằng request.form.get('lop_truc')
    ...
    try:
        ...
        # Kiểm tra bảng có cột Lop_truc không (để build câu INSERT cho đúng schema hiện tại)
        cursor.execute("""
        ...
        has_lop_truc = 'Lop_truc' in cols

        # Kiểm tra id có AUTO_INCREMENT không
        cursor.execute("SHOW COLUMNS FROM accounts LIKE 'id'")
        col = cursor.fetchone() or {}
        has_ai = 'auto_increment' in (str(col.get('Extra') or '')).lower()

        if has_ai:
            # KHÔNG truyền id khi bảng có AUTO_INCREMENT
            if has_lop_truc:
                sql = """
                INSERT INTO accounts
                (`Name`,`Username`,`Password`,`Role`,`Lop`,`Tuan`,`Capquanli`,`Lop_truc`,`Trangthai`)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)
                """
                params = (
                    vals["Name"], vals["Username"], vals["Password"], vals["Role"],
                    vals["Lop"], vals["Tuan"], vals["Capquanli"], vals["Lop_truc"], vals["Trangthai"]
                )
            else:
                sql = """
                INSERT INTO accounts
                (`Name`,`Username`,`Password`,`Role`,`Lop`,`Tuan`,`Capquanli`,`Trangthai`)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
                """
                params = (
                    vals["Name"], vals["Username"], vals["Password"], vals["Role"],
                    vals["Lop"], vals["Tuan"], vals["Capquanli"], vals["Trangthai"]
                )
            cursor.execute(sql, params)
        else:
            # TỰ CẤP id khi chưa AUTO_INCREMENT (khóa bảng tránh trùng id)
            cursor.execute("LOCK TABLES accounts WRITE")
            try:
                cursor.execute("SELECT COALESCE(MAX(id),0)+1 AS next_id FROM accounts")
                next_id = (cursor.fetchone() or {}).get('next_id', 1)

                if has_lop_truc:
                    sql = """
                    INSERT INTO accounts
                    (`id`,`Name`,`Username`,`Password`,`Role`,`Lop`,`Tuan`,`Capquanli`,`Lop_truc`,`Trangthai`)
                    ...
                    sql = """
                    INSERT INTO accounts
                    (`id`,`Name`,`Username`,`Password`,`Role`,`Lop`,`Tuan`,`Capquanli`,`Trangthai`)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)
                    """
                    params = (
                        next_id,
        ...
        conn.commit()
        flash(
            f"Thêm tài khoản thành công! Tên người dùng: **{vals['Username']}**, "
            f"Mật khẩu: **{vals['Password']}**, Tuần: **{vals['Tuan']}**", 'success'
        )

    except mysql.connector.Error as err:
        # Gợi ý debug nhanh tên cột/NOT NULL/STRICT
        flash(f"Lỗi khi thêm tài khoản: {err}", 'error')
        print("ADD_ACCOUNT FAILED:", err)
    finally:
...
def add_accounts_from_excel():
    ...
    try:
        ...
        # 1) Lấy danh sách cột thực tế trong bảng accounts
        cursor.execute("""
            SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'accounts'
        """)
        table_cols = {r['COLUMN_NAME'] for r in cursor.fetchall()}
        has_lop_truc = ('Lop_truc' in table_cols)

        # 2) Kiểm tra cột id có AUTO_INCREMENT không
        cursor.execute("SHOW COLUMNS FROM accounts LIKE 'id'")
        col = cursor.fetchone() or {}
        has_ai = 'auto_increment' in (str(col.get('Extra') or '')).lower()

        # 3) Đọc dữ liệu từ Excel (bắt đầu từ dòng 2)
        rows = []
        for row in sheet.iter_rows(min_row=2, values_only=True):
            # Excel thứ tự: name, username, lop, capquanli
            name, username, lop, capquanli = row
            if not (name and username and lop and capquanli):
                continue
            rows.append({
                ...
                "Password":   generate_specific_password(),
                "Role":       str(capquanli).strip(),   # map role = capquanli như code gốc
                "Lop":        str(lop).strip(),
                "Tuan":       1,
                "Capquanli":  str(capquanli).strip(),
                "Lop_truc":   "",                       # nếu cần, thay bằng giá trị trong file
                "Trangthai":  "Chưa tổng kết",
            })

        if not rows:
            flash("Không có dòng hợp lệ nào trong file Excel.", "warning")
            return redirect(url_for('index'))

        added = 0

        if has_ai:
            # 4A) Bảng đã AUTO_INCREMENT: KHÔNG truyền id
            if has_lop_truc:
                sql = """
                INSERT INTO accounts
                (`Name`,`Username`,`Password`,`Role`,`Lop`,`Tuan`,`Capquanli`,`Lop_truc`,`Trangthai`)
                ...
                """
                params = [
                    (r["Name"], r["Username"], r["Password"], r["Role"],
                     r["Lop"], r["Tuan"], r["Capquanli"], r["Lop_truc"], r["Trangthai"])
                    for r in rows
                ]
            else:
                sql = """
                ...
                params = [
                    (r["Name"], r["Username"], r["Password"], r["Role"],
                     r["Lop"], r["Tuan"], r["Capquanli"], r["Trangthai"])
                    for r in rows
                ]
            # Chèn từng dòng để có thể bỏ qua trùng username (IntegrityError)
            for p in params:
                try:
            ...
            # 4B) Bảng CHƯA AUTO_INCREMENT: tự cấp id, khóa bảng 1 lần
            cursor.execute("LOCK TABLES accounts WRITE")
            try:
                cursor.execute("SELECT COALESCE(MAX(id),0)+1 AS next_id FROM accounts")
                next_id = (cursor.fetchone() or {}).get('next_id', 1)
                ...
                if has_lop_truc:
                    ...
                    """
                    for r in rows:
                        p = (next_id, r["Name"], r["Username"], r["Password"], r["Role"],
                             r["Lop"], r["Tuan"], r["Capquanli"], r["Lop_truc"], r["Trangthai"])
                        try:
                            cursor.execute(sql, p)
                            added += 1
        ...
        conn.commit()
        flash(f"Đã thêm {added} tài khoản từ file Excel.", 'success')

    except Exception as e:
        # In thêm thông tin để debug nhanh nếu cần
        print("IMPORT_EXCEL_FAILED:", repr(e))
        flash(f"Lỗi khi xử lý file Excel: {e}", 'error')
    finally:
        try:
            if cursor: cursor.close()
...
@app.route('/delete_selected_accounts', methods=['POST'])
@login_required
def delete_selected_accounts():
    # chỉ admin mới được xóa nhiều tài khoản
    if session.get('role') != 'admin':
        flash("Bạn không có quyền.", "error")
        return redirect(url_for('index'))

    ids = request.form.getlist('selected_ids')  # nhận danh sách id đã tick trong bảng
    if not ids:
        flash("Bạn chưa chọn tài khoản nào để xóa.", "warning")
        return redirect(url_for('index'))
    ...
    try:
        ...
        sql = f"DELETE FROM accounts WHERE id IN ({qmarks})"
        cur.execute(sql, ids)
        conn.commit()

        flash(f"Đã xóa {cur.rowcount} tài khoản đã chọn.", "success")
    except Exception as e:
