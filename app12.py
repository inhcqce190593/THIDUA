...
def hoc_tap():
    ...
    # Xử lý xóa dữ liệu
    if request.method == 'POST':
        if 'delete_data' in request.form and session.get('role') == 'admin':
            data_id = request.form.get('data_id')
            cursor.execute("DELETE FROM study_data WHERE id = %s", (data_id,))
            conn.commit()
            flash(f"Đã xóa dữ liệu học tập ID {data_id}.", 'success')
        ...
        elif 'delete_all' in request.form and session.get('role') == 'admin':
    ...
    cursor.close()
    conn.close()
    return render_template('hoc_tap.html', study_data=study_data, trangthai_tongket=trangthai_tongket)
...
def add_hoc_tap():
    ...
    user_lop_truc = ''
    if user_lop and user_tuan:
        cursor.execute("SELECT lop_truc FROM phan_cong_truc WHERE lop = %s AND tuan = %s", (user_lop, user_tuan))
        result = cursor.fetchone()
        user_lop_truc = result['lop_truc'] if result else 'Chưa gán'
    cursor.execute("SELECT DISTINCT lop_truc FROM phan_cong_truc WHERE tuan = %s ORDER BY lop_truc", (user_tuan,))
    ...
    cursor.close()
    conn.close()
    return render_template('add_hoc_tap.html', user_lop=user_lop, user_tuan=user_tuan, user_lop_truc=user_lop_truc, role=session.get('role'), available_lop_truc=available_lop_truc)
# Trang Nội Quy
...
def add_noi_quy():
    ...
    available_lop_truc = [row['lop_truc'] for row in cursor.fetchall()]
    if session.get('role') == 'user' and (not user_lop_truc or not user_tuan):
        flash("Bạn cần được gán lớp trực và tuần trước khi thêm dữ liệu vi phạm nội quy.", 'error')
        cursor.close()
        conn.close()
        return redirect(url_for('user'))
    cursor.execute("SELECT trangthai FROM bang_tong_ket WHERE lop_truc = %s AND tuan = %s", (user_lop_truc, user_tuan))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            """, (tuan, user_lop, lop_truc, noi_dung_vi_pham, diem_tru, tong_diem_vi_pham, so_luot_vi_pham, ten_hoc_sinh_vi_pham))
            conn.commit()
    ...
    cursor.close()
    conn.close()
    return render_template('add_noi_quy.html', user_lop=user_lop, user_tuan=user_tuan, user_lop_truc=user_lop_truc, role=session.get('role'), available_lop_truc=available_lop_truc)
...
def tong_ket():
        ...
        if 'recalculate' in request.form:
            if session.get('role') == 'user':
                if trangthai_tongket == 'Đã tổng kết':
                    ...
                    conn.close()
                    return redirect(url_for('tong_ket'))
                if not user_lop_truc or not user_tuan:
                    flash("Bạn cần được gán lớp trực và tuần trước khi tổng kết.", 'error')
                    cursor.close()
                    conn.close()
                    return redirect(url_for('home'))
            ...
            # Xóa dữ liệu tổng kết cũ cho lớp và tuần được chọn
            cursor.execute("DELETE FROM bang_tong_ket WHERE lop_truc = %s AND tuan = %s", (lop_truc, tuan))
            conn.commit()
    ...
    cursor.execute("SELECT DISTINCT khoi FROM bang_tong_ket WHERE khoi IS NOT NULL ORDER BY khoi ASC")
    available_khoi = [row['khoi'] for row in cursor.fetchall()]
    cursor.execute("SELECT DISTINCT lop_truc FROM bang_tong_ket ORDER BY lop_truc ASC")
    available_lop_truc = [row['lop_truc'] for row in cursor.fetchall()]
    ...
    if not data:
        ...
        cursor.close()
        conn.close()
        return render_template('tong_ket.html', data=[], available_weeks=available_weeks, available_khoi=available_khoi, available_lop_truc=available_lop_truc, selected_tuan=selected_tuan, selected_khoi=selected_khoi, trangthai_tongket=trangthai_tongket)
    ...
    cursor.close()
    conn.close()
    return render_template('tong_ket.html', data=ranked_data, available_weeks=available_weeks, available_khoi=available_khoi, available_lop_truc=available_lop_truc, selected_tuan=selected_tuan, selected_khoi=selected_khoi, trangthai_tongket=trangthai_tongket)
